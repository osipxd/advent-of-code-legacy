 ...
 *
 * *[Open in browser](https://adventofcode.com/2020/day/4)*
 */
object Day4 {
    ...
    fun part2(batch: Sequence<String>): Int {
        return validatePassports(batch, ::validateRequiredFields, ::validateFields)
    }
    ...
    fun validateFields(passport: Map<String, String>): Boolean {
        return passport.all { (key, value) ->
            when (key) {
                "byr" -> value in 1920..2002
                "iyr" -> value in 2010..2020
                "eyr" -> value in 2020..2030
                "hgt" -> value.validateHeight()
                "hcl" -> value matches hclRegex
                "ecl" -> value matches eclRegex
    ...
    private fun String.validateHeight(): Boolean {
        val height = dropLast(2)
        return when {
            endsWith("in") -> height in 59..76
            endsWith("cm") -> height in 150..193
            else -> false
        }
    }

    private operator fun IntRange.contains(stringInt: String): Boolean = stringInt.toIntOrNull() in this
}
